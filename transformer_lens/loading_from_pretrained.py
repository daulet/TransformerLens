...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    # TODO address message: This model is using final RMS normalization, so the writing weights can't be centered! Skipping
        ...
        cfg_dict = {
            ...
            "act_fn": hf_config.hidden_act,
            "initializer_range": hf_config.initializer_range,
            "normalization_type": "LN", # our impl does it in fp32
            "positional_embedding_type": "rotary",
            "rotary_base": hf_config.rope_theta,
            "rotary_dim": hf_config.hidden_size // hf_config.num_attention_heads,
            "use_attn_scale": True, # TODO confirm
            "n_key_value_heads": hf_config.num_key_value_heads,
            "final_rms": True, # TODO try without
            "parallel_attn_mlp": True,
            "use_hook_mlp_in": True, # TODO next try withouyt
            # model_max_length
            # logit_scale
        }
    else:
...
def get_pretrained_state_dict(
    ...
    **kwargs,
    ...
    if (
        ...
        elif cfg.original_architecture == "CohereForCausalLM":
            state_dict = convert_cohere_weights(hf_model, cfg)
        else:
            raise ValueError(
